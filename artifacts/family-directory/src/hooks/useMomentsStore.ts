...
interface MomentsState {
  moments: Moment[];
  /** Derived: moments where archived !== true. Always in sync with moments. */
  activeMoments: Moment[];
  isLoaded: boolean;
  createMoment: (input: CreateMomentInput) => Promise<Moment>;
  updateMoment: (
    id: string,
    input: UpdateMomentInput & { newPhotoDataUrls?: string[]; removedPhotoKeys?: string[] }
  ) => Promise<void>;
...
//
// ONE global instance. Every component that calls useMomentsStore() subscribes
// to the same state — no stale closures, no event synchronization hacks.
...
export const useMomentsStore = create<MomentsState>()((set, get) => ({
  ...
  createMoment: async (input) => {
    ...
    try {
      ...
      set({ moments: next, activeMoments: next.filter(m => !m.archived) });
      saveMoments(next);
  ...
  updateMoment: async (id, input) => {
    ...
    const next = moments.map((m) => (m.id === id ? updated : m));
    set({ moments: next, activeMoments: next.filter(m => !m.archived) });
    saveMoments(next);
  },
...
(async function hydrate() {
  ...
  if (local.length > 0) {
    logHydration(
      `Moments loaded from: localStorage (${local.length} moment${local.length !== 1 ? "s" : ""} — showing while GitHub loads)`
    );
    useMomentsStore.setState({
      moments: local,
      activeMoments: local.filter(m => !m.archived),
      isLoaded: true,
    });
  }
  ...
  try {
    ...
    if (remote !== null) {
      if (Array.isArray(remote) && remote.length > 0) {
        ...
        if (_isMomentSaving) {
          ...
          useMomentsStore.setState({
            moments: localNow,
            activeMoments: localNow.filter(m => !m.archived),
            isLoaded: true,
          });
          return;
        }
        ...
        if (localStorage.getItem(GITHUB_HYDRATION_DISABLED_KEY)) {
          logHydration(`Moments: GitHub hydration disabled by debug toggle — keeping ${local.length} local moments`);
          useMomentsStore.setState({ isLoaded: true });
          return;
        }
        ...
        useMomentsStore.setState({
          moments: merged,
          activeMoments: merged.filter(m => !m.archived),
          isLoaded: true,
        });
        saveMoments(merged);
        return;
      ...
      if (_isMomentSaving) {
        const localNow = loadMoments();
        logHydration(
          `Moments: GitHub returned 0 but save is in progress — refreshing to post-save state ` +
          `(${localNow.length} moment${localNow.length !== 1 ? "s" : ""})`
        );
        useMomentsStore.setState({
          moments: localNow,
          activeMoments: localNow.filter(m => !m.archived),
    ...
    logHydration(
      `Moments loaded from: localStorage (${local.length} moment${local.length !== 1 ? "s" : ""} — GitHub unreachable)`
    );
  } catch {
    logHydration(
      `Moments loaded from: localStorage (${local.length} moment${local.length !== 1 ? "s" : ""} — GitHub threw)`
    );
  }
